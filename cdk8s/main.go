...
func NewChart(scope constructs.Construct, id string, env Environment) cdk8s.Chart {
	...
	configMapName := fmt.Sprintf("%s-config", appLabel)
	configMapData := make(map[string]*string)
	for key, value := range env.ConfigMap.Data {
		configMapData[key] = jsii.String(value)
	}
	...
	// Create Deployment with ConfigMap mounted
				...
				Spec: &k8s.PodSpec{
					Containers: &[]*k8s.Container{{
						...
						VolumeMounts: &[]*k8s.VolumeMount{{
							Name:      jsii.String("config-volume"),
							MountPath: jsii.String("/etc/config"),
